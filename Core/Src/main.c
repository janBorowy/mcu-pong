...
int main(void)
{
  ...
  audio_player.hi2s = &hi2s3;
  audio_player_init(&audio_player);
  Game.audio_player = &audio_player;

  Max7219_Config config_left;
  config_left.hspi = &hspi2;
  config_left.gpio_port = LEFT_MATRIX_CS_GPIO_Port;
  config_left.gpio_pin = LEFT_MATRIX_CS_Pin;
  Max7219_Config config_right;
  config_right.hspi = &hspi2;
  config_right.gpio_port = RIGHT_MATRIX_CS_GPIO_Port;
  config_right.gpio_pin = RIGHT_MATRIX_CS_Pin;
  ...
  uint32_t last_game_tick = HAL_GetTick();
  while (1)
  {
    if (HAL_GetTick() - last_game_tick > GAME_FRAME_MS) {
      game_loop(&Game);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, LD2_Pin|LEFT_MATRIX_CS_Pin|RIGHT_MATRIX_CS_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : LD2_Pin LEFT_MATRIX_CS_Pin RIGHT_MATRIX_CS_Pin */
  GPIO_InitStruct.Pin = LD2_Pin|LEFT_MATRIX_CS_Pin|RIGHT_MATRIX_CS_Pin;
